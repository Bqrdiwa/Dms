...
export default function ImageMap({ posX, posY, setPos, setMapId }: ImageMapProps) {
    ...
    const fetchMapData = async () => {
        try {
            const res = await apiClient.get<MapData>(BACKEND_BASE_URL + "/api/map/first/")
    ...
    if (!map) return <Skeleton className="w-1/2 h-full rounded-full" />;
    ...
    return (
        ...
        >
            <img
                ...
                src={map.imageUrl}
                alt={map.name}
                style={{ display: "block", width: "100%", maxWidth: "600px", height: "full" }}
            />
